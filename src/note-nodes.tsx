...
const points: {
  importance: number;
  pullWeights: [number, number, number, number, number, number];
  label: string;
}[] = [
...
export const NoteNodes: React.FC<{ bboxSideLength: number }> = ({
  bboxSideLength,
}) => {
    ...
    useMemo(() => {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        topicNodes.push(
          new Vertex(
            new Vector2d([
              centerDist * (1 + Math.cos(angle)),
              centerDist * (1 + Math.sin(angle)),
            ]),
          ),
        );
      }

      const noteNodes = points.map(() => {
        return new Vertex(null, marginedBboxSideLength);
      });
      ...
      totalNodes.forEach((pt) => {
        ...
        totalNodes.forEach((other) => {
          if (
            ...
            !edges.some(
              (edge) =>
                (edge.start === pt && edge.end === other) ||
                (edge.start === other && edge.end === pt),
            )
          ) {
  ...
  return (
    <svg
      width={bboxSideLength}
      height={bboxSideLength}
      viewBox={`0 0 ${bboxSideLength} ${bboxSideLength}`}
    >
      {topicNodes.map((vertex, i) => (
        <circle
          key={i}
          cx={vertex.pos.x + margin}
          cy={vertex.pos.y + margin}
          r={vertexRadius}
          fill={colors[i]}
        />
      ))}
      ...
      {edges.map((edge, i) => (
        <line
          key={i}
          x1={edge.start.pos.x + margin}
          y1={edge.start.pos.y + margin}
          x2={edge.end.pos.x + margin}
          y2={edge.end.pos.y + margin}
